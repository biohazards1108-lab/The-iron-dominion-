...
public final class DominionTokens extends JavaPlugin {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (args.length == 0 || args[0].equalsIgnoreCase("balance")) {
            ...
            if (target == null) {
                sender.sendMessage("Usage: /dt balance <player>");
                return true;
            }
        ...
        if (args.length != 3) {
            sender.sendMessage("Usage: /dt <add|take|set> <player> <amount>");
            return true;
        }
        ...
        if (args[0].equalsIgnoreCase("add")) ok = changeBalance(target, amount);
            ...
            if (amount > 1000000000L) ok = false;
            else { balances.put(keyFor(target), Long.valueOf(amount)); saveBalances(); ok = true; }
        } else {
            sender.sendMessage("Usage: /dt <add|take|set> <player> <amount>");
            return true;
        }
    ...
    private void loadBalances() {
        ...
        try (java.io.FileInputStream in = new java.io.FileInputStream(dataFile)) {
            ...
            for (String k : p.stringPropertyNames()) {
                try { balances.put(k, Long.valueOf(p.getProperty(k))); } catch (NumberFormatException ignored) { }
            }
    ...
    private void saveBalances() {
        ...
        try (java.io.FileOutputStream out = new java.io.FileOutputStream(dataFile)) {
            p.store(out, "Iron Dominion Dominion Token balances");
